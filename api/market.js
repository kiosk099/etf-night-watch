...
async function daily(code) {
  ...
  return cached(`naver:${code}`,async()=>{
    const j=await fetchJson(url,{timeoutMs:3500,headers:{Referer:`https://m.stock.naver.com/domestic/stock/${code}/total`}});
...
async function chart(symbol, range='5d') {
  ...
  return cached(`yahoo:${symbol}:${range}`,async()=>{
    const j=await fetchJson(url,{timeoutMs:4500}), r=j.chart?.result?.[0];
...
async function loadCharts(symbols, range='5d', concurrency=6) {
